...
# ============================================================================
"""global args for Visual Attention Network"""
import argparse
import ast
import os
import sys
...
def parse_arguments():
    """parse_arguments"""
    global args
    parser = argparse.ArgumentParser(description="MindSpore Visual Attention Network Training")

    parser.add_argument("-a", "--arch", metavar="ARCH", default="van_base_224",
    ...
    parser.add_argument("--best_every", default=10, type=int, help="Move model best every epochs(default:5)")
    parser.add_argument("--save_every", default=50, type=int, help="Save model ckpt every epochs(default:50)")
...
def get_config():
    ...
    if args.run_openi and args.pretrained is not None:
        current_path = os.path.abspath(__file__)
        src_dir = os.path.dirname(current_path)
        pretrained_file_path = os.path.join(src_dir, "ckpt/{}{}.ckpt".format(args.arch, args.pretrained))
        if not os.path.exists(pretrained_file_path):
            raise ValueError("pretrained file: {} not exists!".format(pretrained_file_path))
        args.pretrained = pretrained_file_path
