...
def keep_best_latest_models(best_model_dir, num_keep=5):
    ...
    if not best_model_list:
        ...
        best_model_list = sorted(best_model_list,  key=lambda x: os.path.getmtime(os.path.join(best_model_dir, x)), reverse=True)
...
class EvaluateCallBack(Callback):
    ...
    def __init__(self, model, eval_dataset, src_url, train_url, rank, model_prefix, batch_num, best_model_dir, 
    best_freq=5, save_freq=50):
    ...
    def epoch_end(self, run_context):
        ...
        if args.run_modelarts or args.run_openi:
